...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
    return (
        <motion.div
            ...
            className={cn(
                "flex relative z-[60] w-full flex-row items-center justify-between rounded-lg bg-transparent px-4 py-4 bg-transparent mt-0",
                visible && "bg-white/80 backdrop-blur-md bg-neutral-950/80 px-3 py-3 mt-0",
                className
            )}
        >
            {children}
            ...
            <div className="backdrop-blur-sm bg-white/15 rounded-md py-[5px] glass">
                {items.map((item, idx) => (
                    <a
                        ...
                        onClick={onItemClick}
                        className="relative px-2 py-1 text-neutral-600 text-neutral-300 mx-2 "
                        key={`link-${idx}`}
                        href={item.link}
                    >
                        {hovered === idx && (
                        ...
                        <span className="relative z-20 text-white text-sm">{item.name}</span>
                    </a>
                ))}
            </div>
        </motion.div>
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
    return (
        <motion.div
            ...
            className={cn(
                "rounded-lg relative z-50 mx-auto flex max-w-full flex-col items-center justify-between bg-transparent px-0 py-2 lg:hidden",
                visible && "bg-neutral-950/80 px-3 py-3 mt-0",
                className,
            )}
        >
            {children}
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    exit={{ opacity: 0 }}
                    className={cn(
                        "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-sm bg-white px-4 py-8 shadow-[0_0_24px_rgba(34,_42,_53,_0.06),_0_1px_1px_rgba(0,_0,_0,_0.05),_0_0_0_1px_rgba(34,_42,_53,_0.04),_0_0_4px_rgba(34,_42,_53,_0.08),_0_16px_68px_rgba(47,_48,_55,_0.05),_0_1px_0_rgba(255,_255,_255,_0.1)_inset] backdrop-blur-md bg-neutral-950/80 text-white",
                        className,
                    )}
                >
                    {children}
